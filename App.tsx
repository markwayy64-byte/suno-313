...
import { Mic, Send, Disc, Activity, AlertCircle, Brain, Globe, StopCircle, Menu, Paperclip, Settings, Hash, LayoutTemplate, X, AlertTriangle, Library } from 'lucide-react';
...
import { PRESETS, BEEF_UP_PRESETS, AUTOSUGGEST_TAGS, CONFLICTING_TAGS } from './constants';
...
const App: React.FC = () => {
  ...
  const [tagConflict, setTagConflict] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    for (const [tag, conflicts] of Object.entries(CONFLICTING_TAGS)) {
        if (lowerInput.includes(tag)) {
            for (const conflict of conflicts) {
                if (lowerInput.includes(conflict)) {
                    foundConflict = `${tag.toUpperCase()} contradicts ${conflict.toUpperCase()}`;
                    break;
                }
            }
        }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Get Audio Duration using Web Audio API (Reliable check)
    let duration = 0;
    try {
    ...
    reader.onloadend = async () => {
        ...
        try {
            // Trigger Real D-Hz "Analysis" using Multimodal Gemini
            const analysis = await analyzeAudioContext(
                file.name, 
                duration, 
                base64Audio,
                file.type // Pass detected MIME type
            );
            ...
            const errorMsg: Message = {
                ...
                role: 'system',
                type: 'error',
                content: 'AUDIO ANALYSIS FAILED. ENGINE ERROR.',
                timestamp: Date.now()
            };
            setMessages(prev => [...prev, errorMsg]);
        } finally {
      ...
      <footer className="w-full max-w-4xl p-4 sticky bottom-0 z-20 bg-[#050505]">
        ...
        <div className="flex gap-4 mb-2 justify-end items-center">
           ...
           <button 
             ...
             className={`flex items-center gap-1 text-[10px] font-mono px-2 py-1 rounded border transition-colors ${showPresets ? 'bg-detroit-steel border-gray-500 text-white' : 'border-gray-800 text-gray-600 hover:border-gray-600'}`}
             title="Load Preset"
           >
             <Library className="w-3 h-3" /> PRESETS
           </button>
        ...
        <div className="relative">
            ...
            {tagConflict && (
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-4 w-full max-w-sm">
                    <div className="bg-yellow-900/80 border border-yellow-500/50 text-yellow-200 px-4 py-2 rounded-lg text-xs font-mono flex items-center justify-center gap-2 shadow-lg backdrop-blur-md animate-in fade-in slide-in-from-bottom-2">
                        <AlertTriangle className="w-4 h-4" />
                        <span>CONFLICT DETECTED: {tagConflict}</span>
                    </div>
                </div>
            )}
            ...
            {showTemplateEditor && (
                <div className="absolute bottom-full right-0 mb-2 w-full max-w-md bg-detroit-dark border border-detroit-neon/30 rounded-lg shadow-xl z-50">
                     <TemplateEditor 
            ...
            {/* Presets Popup */}
            {showPresets && (
                <div className="absolute bottom-full right-0 mb-2 w-64 max-h-60 overflow-y-auto bg-detroit-dark border border-detroit-neon/30 rounded-lg shadow-xl z-50">
                    <div className="p-2">
                        <h3 className="text-[10px] font-mono font-bold text-gray-500 mb-2 px-2">
                            {mode === AppMode.GENERATOR ? 'GENRE PROTOCOLS' : 'SIGNAL CHAINS'}
                        </h3>
                        {(mode === AppMode.GENERATOR ? PRESETS : BEEF_UP_PRESETS).map((p, i) => (
                            <button
                                key={i}
                                onClick={() => applyPreset(p)}
                                className="w-full text-left px-2 py-2 hover:bg-gray-800 rounded group"
                            >
                                <div className="text-xs font-bold text-gray-300 group-hover:text-white">{p.genre}</div>
            ...
            {showSuggestions && filteredSuggestions.length > 0 && (
                <div className="absolute bottom-full left-0 mb-2 w-64 max-h-48 overflow-y-auto bg-detroit-dark border border-detroit-neon/30 rounded-lg shadow-xl z-50">
                    <div className="p-1">
                        {filteredSuggestions.map((tag) => (
                            <button
                                key={tag}
                                onClick={() => insertTag(tag)}
                                className="w-full text-left px-3 py-2 text-xs font-mono text-gray-300 hover:bg-detroit-steel hover:text-detroit-neon rounded flex items-center gap-2"
                            >
            ...
            <div className={`relative border rounded-lg bg-[#0a0a0a] shadow-[0_0_20px_rgba(0,0,0,0.5)] transition-colors ${tagConflict ? 'border-yellow-500/50' : 'border-gray-800 focus-within:border-detroit-neon/50'}`}>
                <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-detroit-neon/20 to-transparent"></div>
                ...
                <textarea
                    ...
                    placeholder={isRecording ? "Listening..." : mode === AppMode.BEEF_UP ? "Describe your uploaded beat... (Type '[' for tags)" : "Describe the track you want to engineer... (Type '[' for tags)"}
                    disabled={isRecording}
                    className="w-full bg-transparent text-sm p-4 pr-32 outline-none h-16 resize-none font-mono text-gray-300 placeholder-gray-700"
                />
                ...
                <div className="absolute right-2 bottom-2 flex gap-2">
                    <button
                        ...
                        disabled={isRecording}
                        className="p-2 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white rounded transition-all disabled:opacity-50"
                    ...
                    <button
                        ...
                        className={`p-2 rounded text-white transition-all ${isRecording ? 'bg-red-500 animate-pulse' : 'bg-gray-800 hover:bg-gray-700'}`}
                    ...
                    <button 
                        ...
                        className="p-2 bg-detroit-steel hover:bg-detroit-neon text-white hover:text-black rounded transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
        ...
        {!hasKey && (
             <div className="mt-2 text-center">
                 <p className="text-[10px] text-red-500 font-mono flex items-center justify-center gap-1">
                     <AlertCircle className="w-3 h-3" /> SYSTEM OFFLINE: API_KEY MISSING
                 </p>
             </div>
        )}
        <div className="mt-2 flex justify-between items-center text-[10px] text-gray-700 font-mono">
            <span>STATUS: {isLoading ? 'PROCESSING' : isRecording ? 'RECORDING' : 'ONLINE'}</span>
            <span>FIDELITY: {sampleRate} / {bitDepth}</span>
        </div>
      </footer>
    </div>
  );
